...
def load_test_instances():
    ...
    test_filepath = os.getcwd() + '/data/processed_data1.csv'
    test_data = np.loadtxt(test_filepath, delimiter=',')

    doc_ids = (test_data[:, 0].reshape(num_classes,128))[:, 0]
    doc_labels = (test_data[:, 1].reshape(num_classes,128))[:, 0]
    sensory_data = test_data[:, 2:].reshape(num_classes,6,128)
...
async def predict_Activity(request: Request, doc_id: int=Form(...)):
    ...
    activity = utils.glda_clustering(main_df)
